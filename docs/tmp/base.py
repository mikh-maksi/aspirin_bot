...
from step_dicts import start_step, main_step, document_step, stage_check, getemail, start_code_check, get_list
from question_text import aspirin_description
...
start_options = start_step()
main_options = main_step()
document_options = document_step()
...
def keyb(id,list_in):
    ...
    if stage_check(id) == 2:
        ...
        for i in range(len(get_list(start_step()))):
            if q_lst[i]==0:
                key_lst = [(InlineKeyboardButton(start_step()[i]["name"], callback_data=start_step()[i]["slug"]))]
                kb.append(key_lst)
...
def start(update: Update, context: CallbackContext) -> None:
    ...
    chat = update.effective_chat
    print(chat.id)
    keyboard = keyb(int(chat.id),start_options)
    reply_markup = InlineKeyboardMarkup(keyboard)
